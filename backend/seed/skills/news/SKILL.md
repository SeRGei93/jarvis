...
allowed-tools: search_news web_fetch web_search
...
3. **Read descriptions** — `search_news` returns headlines with optional snippets. Many items have empty descriptions — only a headline and URL. For items you want to include in the digest, call `web_fetch` on 2–3 most interesting article URLs to get real content for meaningful summaries. Without fetching, you can only restate the headline.
4. **Call web_search** — only if user asked about a specific topic not covered by search_news results.
5. **web_fetch each URL from web_search** — mandatory before citing any web_search result.
...
- **web_fetch** — max 10 calls, required for web_search URLs
...
- **web_fetch fails:** Discard URL, move to next
- **No search_news results:** Try web_search with broader query
- **All web_fetch failed:** Explain why (paywall, timeout), suggest alternatives
