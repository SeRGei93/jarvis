...
- [ ] **18. Структурированная выдача новостей с медиа** — переработать новостной инструмент (`search_news`) и парсеры так, чтобы новости отдавались **структурированно, с картинками**, а не плоским markdown-текстом с ad-hoc строками `Фото:`. Цель: тул возвращает типизированные элементы (`title`/`url`/`image`/`source`/`date`/`description`), а **слой доставки** (Telegram-адаптер) сам решает, **как** показать (карточка/медиа-группа/коллаж) и **когда** показывать фото (детальный просмотр vs лента) — поведение в коде инструмента/адаптера, **не в промптах** (промпты остаются простыми). Учесть: webp от onliner (рендер в Telegram), отсев заглушек/lazy, лимиты медиа. **Задел готов** (M-этого-этапа): извлечение фото на уровне парсеров — `NewsItem.image` (лента) и `NewsArticle.image` (статья: onliner `news-header__image` + og:image у tochka/smartpress/realt), общий `extractImageUrl`/`collectImageUrls` с отсевом плейсхолдеров и приоритетом lazy-атрибутов. Осталось: спроектировать структурный контракт `search_news`/`fetch_url` + рендер медиа в адаптере.
