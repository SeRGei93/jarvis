...
## Status
Migration **milestones 0–6 complete** (каркас, БД+настройки, LLM-слой, память, скилы+chat workflow, инструменты+MCP, Telegram-бот). Source of truth: `.ai-factory/ROADMAP.md` (milestones 0→10). Next: **M7** — Cron-планировщик (node-cron исполнитель + нотификации). M6 landed: grammY-бот (`backend/src/telegram/`) — polling по умолчанию (+ опц. минимальный webhook), throttled `editMessageText`-стриминг, markdown→MarkdownV2 (`marked`) + сплит по 4096, голос→speech, команды (`/start /help /new /me /tasks /usage /about /reset_onboarding`), get-or-create user/channel, allowlist, `Messenger` под cron-нотификации.
...
- **Cron:** `cron-parser` (schedule validation, M5) + node-cron *(execution, M7)* · **Telegram:** grammY (polling + опц. webhook, throttled streaming), markdown→MarkdownV2 via `marked` · **HTTP/Admin:** Hono *(M8)*
